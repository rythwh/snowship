...
namespace Snowship.NEditor
{
	...
	public sealed class EntityDefsEditorWindow : EditorWindow
	{
		...
		[MenuItem("Snowship/Entity JSON Editor")]
		public static void Open()
		{
			EntityDefsEditorWindow win = GetWindow<EntityDefsEditorWindow>(true, "Entity JSON Editor");
		...
		private void DrawEditorPane()
		{
			...
			for (int i = 0; i < current.Components.Count; i++) {
				JsonComponent c = current.Components[i];
		...
		private void DrawComponentEditor(JsonComponent c)
		{
		...
		private void DrawBaseStats(JsonComponent c)
		{
		...
		private void DrawBaseRecipe(JsonComponent c)
		{
		...
		private void DrawMaterialOptions(JsonComponent c)
		{
		...
		private void DrawOverrideMap(JObject map)
		{
			...
			foreach (string key in keys) {
				...
				string mode = ov.TryGetValue("mode", out JToken modeToken) ? modeToken.Value<string>() : "ReplaceAll";
		...
		{
			if (parent[key] is not JArray arr) {
				arr = new JArray();
				parent[key] = arr;
			}
		...
		{
			try {
				JsonEntity def = JsonIO.Load<JsonEntity>(filePaths[index]);
				current = def;
				selectedIndex = index;
			} catch (Exception ex) {
				Debug.LogError(ex);
		...
		private void AddBaseStats()
		{
			...
			JsonComponent comp = new() { Type = "BaseStats", Data = stats };
		...
		private void AddMaterialOptions()
		{
			...
			JsonComponent comp = new() { Type = "MaterialOptions", Data = obj };
			current.Components.Add(comp);
		}
