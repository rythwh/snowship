...
public class DebugManager : IStartable, IPostStartable, ITickable
{
	...
	private void CreateCommandFunctions() {
				...
				if (parameters.Count == 1) {
					...
					if (selectedHuman != null) {
						...
						if (humanM.selectedHuman == selectedHuman) {
							Output($"SUCCESS: Selected {humanM.selectedHuman.Name}.");
						}
					} else {
					...
					if (int.TryParse(parameters[0], out int numberToSpawn)) {
						...
						if (int.TryParse(parameters[2], out int maxNumTraders)) {
							...
							if (oldNumCaravans + numberToSpawn == caravanM.caravans.Count) {
								...
								Output($"ERROR: Unable to spawn caravans. Spawned {caravanM.caravans.Count - oldNumCaravans} caravans.");
							}
						} else {
					...
					if (float.TryParse(parameters[0], out float newHealth)) {
						if (newHealth is >= 0 and <= 1) {
							if (humanM.selectedHuman != null) {
								...
								if (field != null) {
									field.SetValue(humanM.selectedHuman, 100);
								}
								if (Mathf.Approximately(humanM.selectedHuman.Health, newHealth)) {
									Output($"SUCCESS: {humanM.selectedHuman.Name}'s health is now {humanM.selectedHuman.Health}.");
								} else {
									Output($"ERROR: Unable to change {humanM.selectedHuman.Name}'s health.");
								}
							} else {
		...
		commandFunctions.Add(
			...
			delegate(List<string> parameters) {
				if (parameters.Count == 2) {
					...
					if (needPrefab != null) {
						if (humanM.selectedHuman != null) {
							Human human = humanM.selectedHuman;
		...
		commandFunctions.Add(
			...
			delegate(List<string> parameters) {
				if (parameters.Count == 1) {
					...
					if (moodPrefab != null) {
						if (humanM.selectedHuman != null) {
							if (humanM.selectedHuman is Colonist) {
								Colonist selectedColonist = (Colonist)humanM.selectedHuman;
								selectedColonist.Moods.AddMoodModifier(moodPrefab.type);
								Output($"SUCCESS: Added {moodPrefab.name} to {humanM.selectedHuman}.");
							} else {
						...
						if (humanM.selectedHuman != null /* || UIManagerOld.selectedContainer != null*/) {
							if (humanM.selectedHuman != null) {
								if (int.TryParse(parameters[1], out int amount)) {
									humanM.selectedHuman.Inventory.ChangeResourceAmount(resource, amount, false);
									Output($"SUCCESS: Added {amount} {resource.name} to {humanM.selectedHuman}.");
								} else {
							...
							/*if (UIManagerOld.selectedContainer != null) {
								if (int.TryParse(parameters[1], out int amount)) {
									UIManagerOld.selectedContainer.Inventory.ChangeResourceAmount(resource, amount, false);
				...
				if (parameters.Count == 1) {
					if (int.TryParse(parameters[0], out int index)) {
						if (index >= 0 && index < Container.containers.Count) {
							// UIManagerOld.SetSelectedContainer(Container.containers[index]);
						} else {
		...
		commandFunctions.Add(
			...
			delegate(List<string> parameters) {
				if (parameters.Count == 0) {
					holdHour = timeM.Time.DecimalHour;
		...
		// 	delegate(List<string> parameters) {
		// 		if (parameters.Count == 0) {
		// 			foreach (Tile tile in mapM.Map.DetermineShadowSourceTiles(mapM.Map.tiles)) {
		// 				tile.SetVisible(true);
		// 				tile.sr.color = Color.red;
		// 			}
		// 		} else {
